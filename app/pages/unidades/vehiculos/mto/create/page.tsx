...
const CrearMantenimiento: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    camion_id: camionId, // Mantiene el camion_id
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await axios.post('/api/auth/mantenimiento-camion', mantenimientoData, {
            headers: {
          ...
          if (data) {
            Swal.fire('Guardado', 'El mantenimiento se ha guardado correctamente.', 'success').then(() => {
              router.push(`/pages/unidades/vehiculos/mto?id=${camionId}`);
            });
          } else {
  ...
  return (
    <div className="panel">
      ...
      <div className="panel-header">
        <div className="title">
          Agregar un mantenimiento para el camión Nº {camionId}
