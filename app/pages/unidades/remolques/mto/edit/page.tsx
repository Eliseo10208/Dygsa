...
const EditarMantenimientoRemolque: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMantenimiento = async () => {
      try {
        const response = await axios.get(`/api/auth/mantenimiento-remolque/${mantenimientoId}`);
        const data = response.data[0];

        // Formatear la fecha para que sea compatible con el selector de fecha
        const formattedDate = data.fecha_mantenimiento ? new Date(data.fecha_mantenimiento).toISOString().split('T')[0] : '';

        setFormData({
          remolque_id: data.remolque_id,
          tipo_mantenimiento: data.tipo_mantenimiento,
          descripcion: data.descripcion,
          fecha_mantenimiento: formattedDate,
          costo: data.costo,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await axios.put(`/api/auth/mantenimiento-remolque/${mantenimientoId}`, mantenimientoData, {
            headers: {
          ...
          if (response.data) {
            Swal.fire('Guardado', 'El mantenimiento se ha actualizado correctamente.', 'success').then(() => {
              router.push(`/pages/unidades/remolques/mto?id=${formData.remolque_id}`);
            });
          } else {
...
export default EditarMantenimientoRemolque;
